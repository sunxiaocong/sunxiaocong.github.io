---
layout: post
title: "Rsyslog to ElasticSearch 配置"
date: 2019-01-21
categories: 大数据
tags: [数据采集,rsyslog,ElasticSearch]
image: http://gastonsanchez.com/images/blog/mathjax_logo.png
---

ryslog 是一个快速处理收集系统日志的程序，提供了高性能、安全功能和模块化设计。rsyslog 是syslog 的升级版，它将多种来源输入输出转换结果到目的地，据官网介绍，现在可以处理100万条信息。
Github[地址](https://github.com/daijiangtian/rsyslog_client) 该为Python版本往Rsyslog的SDK，
该SDK与Python自身Loging模块相结合，通过队列批发高峰写文件的机制减轻高峰时期Rsyslog服务的压力。

<!-- more -->

### TCP to Rsyslog to ElasticSearch
...
### FILE TO RSYSLOG TO ELASTIC

```conf
# 配置静态文件路径，必须确保该路径可写
$WorkDirectory /data/statefile
module(load="builtin:omfile") # output to file 模块，该demo没有使用
module(load="imfile") # input from file模块
module(load="mmjsonparse") # json parse模块
module(load="omelasticsearch") # output to es模块

# 模板，name用于标识，字符串类型，值等于.suffix变量截取拼接，其中F:代表截取方式，46代表.号的ascii码,0:代表字符串的开始位置,1代表匹配的序号,--end代表结束的位置，end代表全部，
# 参考链接https://www.rsyslog.com/doc/v8-stable/configuration/property_replacer.html。

template(name="Index"
             type="string"
             string="%.suffix:F,46,0:1,-–end%-%.suffix:F,46,0:2,-–end%")

# list类型,拼接方式，constant值为常量,property值为变量，其中all-json代表所有字段，$!代表从json获取，$!tag代表获取json中key为tag的值，timegenerated系统内置变量
# 参考 https://www.rsyslog.com/doc/master/configuration/properties.html
template(name="logFormat" type="list") {
         constant(value="{ ")
         constant(value="\"@timestamp\":\"")
         property(name="timegenerated" dateFormat="rfc3339" )
...
/* input输入源
* type指定为imfile（一开始导入的module）
* File：监控的路径，支持通配符
* Tag：标识数据来自来个文件的标识，目前没有用途
* PersistStateInterval：默认0，代表文件操作符关闭的时候，更新statefile，如果设置整数，代表处理几行，更新一次statfile文件
* addMetadata：额外的信息put到message，filename，offset
* ruleset：绑定的监听事件。输入完后触发
* 其他可以参照文档 https://www.rsyslog.com/doc/v8-stable/configuration/modules/imfile.html
*/

input (
 type="imfile"
 File="/data/rsyslog_test/*.tmp"
 Tag="testlog02"
...
/*
set $.suffix 设置一个变量（不存储到json），通过文件名$!metadata!filename获取到绝对路径/data/rsyslog_test/xxx.xxx.tmp正则截取basename：xxx.xxx.tmp
action(type="mmjsonparse" cookie="")，字符串转为json对象
action(type="omelasticsearch") # output es
* server: es集群地址,多个逗号分隔，使用9200端口，flume使用9300端口，踩过坑
* template：模板，如果需要增加字段json，可以使用
* dynSearchIndex：使用动态索引，设置这个值为on的话searchIndex的值，会使用template的模块，默认是off，值为固定值，静态的
* 其他参考 https://www.rsyslog.com/doc/v8-stable/configuration/modules/omelasticsearch.html
*/
ruleset (name="chat"){
  set $.suffix=re_extract($!metadata!filename, "(.*)/([^/]*)", 0, 2, "rsyslog.log");
  action(type="mmjsonparse" cookie="")

  action(
    type="omelasticsearch"
    server=["http://192.168.5.45:9200", "http://192.168.5.46:9200", "http://192.168.5.47:9200"]
    template="logFormat"
    dynSearchIndex="on"
    searchIndex="Index"
    searchType="rsyslog"
    bulkmode="on"
    maxbytes="100m" # depending on elasticsearch http.max_content_length
    queue.type="linkedlist"
    queue.size="800000"
